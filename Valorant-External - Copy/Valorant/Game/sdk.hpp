...
namespace Globals
{
	...
	FVector MyRelativeLocation, RandomPawnRelativeLocation, PawnLikeMeLocation, closestPawn;
...
namespace UE4
{
	struct GWorld
	{
		...
		uintptr_t gamestate(uintptr_t World) {
			return driver.read<uintptr_t>(World + 0x0210);
		};
	};
	...
	struct GPrivatePawn {
		uintptr_t USkeletalMeshComponent(uintptr_t Pawn) {
			...
			auto ChildActorComponent = driver.read<uintptr_t>(Pawn + 0x510);

			auto ChildActor = driver.read<uintptr_t>(ChildActorComponent + 0x0478);

			return driver.read<uintptr_t>(ChildActor + offsets::MeshComponent);
		};
	};
	...
	struct GPawn {
		auto TeamID(uintptr_t APawn) -> int {
			...
			int team_id = driver.readv<bool>(PlayerState + 0x05A0);
			return team_id;
		};
		...
		auto bIsAbot(uintptr_t APawn) -> bool {
			return driver.readv<bool>(APawn + 0x047C);
		};

		auto Health(uintptr_t APawn) -> float {
			
			return driver.readv<float>(APawn + 0x01C0);
		};
	};
	...
	namespace SDK
	{
		auto GetEntityBone(DWORD_PTR mesh, int id) -> FVector
		{
			...
			FTransform bone = driver.readv<FTransform>(array + (id * 0x60));
		...
		auto ProjectWorldToScreen(FVector WorldLocation) -> FVector
		{
			...
			auto ViewInfo = driver.readv<FMinimalViewInfo>(PlayerCameraManager + 0x1A30 + 0x10);
